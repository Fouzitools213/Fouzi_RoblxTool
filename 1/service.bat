...
for /f "delims=" %%A in ('powershell -command "(Invoke-WebRequest -Uri \"%LICENSE_URL%\" -UseBasicParsing).Content"') do (
    if /I "%%A"=="%PC_NAME%" set AUTHORIZED=1
)
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%winws.exe\" !ARGS!" DisplayName= "Fouzi" start= auto
...
set "GITHUB_VERSION_URL=rcontent.com/Flowseal/zapret-discord-youtube/main/.service/version.txt"
